    ...
    <div class="right">
        ...
        <div class="rightb">
            ...
            <div class="rightg">
                上一篇：
                @if($proInfoPN['pre'])
                    <a href="{{url('product-3-'.$proInfoPN['pre']->art_id).'-type1.html'}}">{{$proInfoPN['pre']->art_title}}</a><br>
                @else
                    <span>没有上一篇了...</span><br/>
                @endif
                下一篇：
                @if($proInfoPN['next'])
                    <a href="{{url('product-3-'.$proInfoPN['next']->art_id).'-type1.html'}}">{{$proInfoPN['next']->art_title}}</a>
                @else
                    <span>没有下一篇了...</span>
                @endif
            </div>
